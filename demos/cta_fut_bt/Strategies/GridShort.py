...
class GridShortStra(BaseCtaStrategy):
    ...
    def on_calculate(self, context:CtaContext):
        ...
        if curPos == 0:
            if (ma_short_days1 < ma_long_days1) and (ma_long_days2 < ma_short_days2):  # 当前未持仓且日线死叉时作为基准价格进场
                # 做多50%资金的手数，为了不使杠杆率过高，总资金乘以一手期货的保证金率再计算手数
                context.stra_enter_short(code, math.floor(self.cur_money*margin_rate*0.5/trdUnit_price), 'entershort')
            ...
            if curPrice / self.benchmark_price > price_list[-1]:
                target_pos = 0
                context.stra_exit_short(code, context.stra_get_position(code), 'exitshort')
            ...
            if target_pos > self.grid_pos:
                context.stra_enter_short(code, math.floor((target_pos-self.grid_pos)*self.cur_money/trdUnit_price), 'entershort')
            ...
            # 当目标仓位小于当前实际仓位时平多
                ...
                context.stra_exit_short(code, math.floor((self.grid_pos-target_pos)*self.cur_money/trdUnit_price), 'exitshort')
